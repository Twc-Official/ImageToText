...
class TextCaptureApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Text Capture Tool v3")
        ...
        self.tabs.addTab(self.build_recent_tab(), "Recent Captures")  # NEW
    ...
    def build_capture_tab(self):
        ...
        self.open_btn = QPushButton("Open Image File")  # NEW
        self.open_btn.clicked.connect(self.open_image_file)
        ...
        self.translate_combo.addItems(["None", "de", "fr", "es", "it", "zh-CN"])
        self.translate_combo.currentTextChanged.connect(self.set_translation_lang)
        translate_btn = QPushButton("Translate Text")
    ...
    def build_recent_tab(self):
        ...
        self.recent_list.itemClicked.connect(self.load_recent_text)
    ...
    def build_about_tab(self):
        ...
        github_edit = QLineEdit("https://github.com/Twc-Official/ImageToText")
    ...
    def open_image_file(self):  
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Image", "", "Image Files (*.png *.jpg *.jpeg *.bmp *.tif)")
        if file_path:
            img = Image.open(file_path)
            self.ocr_complete(img)
    ...
    def show_toast(self, message):
        toast = Toast(message, parent=self)
        # NEW: position bottom right
        geo = self.geometry()
        x = geo.x() + geo.width() - toast.width() - 20
        y = geo.y() + geo.height() - toast.height() - 20
        toast.move(x, y)
        toast.show()

    def set_translation_lang(self, lang):  
        self.translate_lang = None if lang == "None" else lang

    def translate_text(self):  
        text = self.text_output.toPlainText()
        if not text.strip():
            self.show_toast("No text to translate.")
            return
        if not self.translate_lang:
            self.show_toast("Translation disabled.")
            return
        try:
            translated = GoogleTranslator(source='auto', target=self.translate_lang).translate(text)
            self.text_output.setPlainText(translated)
            self.show_toast(f"Translated to {self.translate_lang}")
        except Exception as e:
            self.show_toast(f"Translation error: {e}")

    def add_to_recent(self, text):  
        if text.strip():
            self.recent_captures.insert(0, text)
            self.recent_list.insertItem(0, text[:50] + "..." if len(text) > 50 else text)
            if len(self.recent_captures) > 10:
                self.recent_captures.pop()
                self.recent_list.takeItem(10)

    def load_recent_text(self, item):  
        index = self.recent_list.row(item)
        self.text_output.setPlainText(self.recent_captures[index])
        self.show_toast("Loaded from recent captures.")
    ...
    def search_text(self, keyword):  
        ...
        cursor.setPosition(0)
        while True:
            cursor = self.text_output.document().find(keyword, cursor)
            if cursor.isNull():
                break
...
# --- Screen Capture Overlay --- (There were like 200 bugs here because of a typo)
